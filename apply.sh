...
original_author="shamanthdivakar"
original_name="firstflaskproject"
original_urlname="Firstflaskproject"
original_description="Awesome firstflaskproject created by shamanthdivakar" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/firstflaskproject "${name}"
...
then
    ...
    echo "then run 'firstflaskproject' or 'python -m firstflaskproject'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'firstflaskproject' or 'python -m firstflaskproject' or 'poetry run firstflaskproject'"
fi
