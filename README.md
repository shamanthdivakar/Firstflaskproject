...
- 🎯 Entry points to execute your program using `python -m <firstflaskproject>` or `$ firstflaskproject` with basic CLI argument parsing.
...
---
# firstflaskproject Flask Application

Awesome firstflaskproject created by shamanthdivakar
...
```bash
git clone https://github.com/shamanthdivakar/Firstflaskproject firstflaskproject
...
```bash
firstflaskproject create-db   # run once
firstflaskproject populate-db  # run once (optional)
firstflaskproject add-user -u admin -p 1234  # ads a user
firstflaskproject run
```
